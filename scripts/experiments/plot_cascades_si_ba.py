"""
scripts/experiments/plot_cascades_si_ba.py
Show example cascades for SI on BA across different R0 values.
"""
...
def main() -> None:
    ...
    fig.suptitle(
        f"SI on BA — Cascade Examples (size={CASCADE_SIZE})\n"
        f"R0=0.5 (weak) | R0=1.0 (critical) | R0=2.0 | R0=3.0 (strong) | R0=5.0 (explosive)",
        color="white", fontweight="bold", fontsize=16, y=1.02
    )
    plt.tight_layout()

    out_path = Path("results/figures/cascades/examples_si_ba.png")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_path, dpi=150, facecolor=fig.get_facecolor(), bbox_inches="tight")
    plt.close(fig)
    print(f"Saved SI on BA examples -> {out_path}")
