"""ba network figure on white background: spring-layout graph + power-law degree histogram"""
import sys
...
def main() -> None:
    ...
    # label the top-5 hubs — they dominate the preferential-attachment dynamics
    ...
    # right panel: degree distribution on log-log to show the scale-free tail
    deg_vals = list(degrees.values())
    unique_degs, counts = np.unique(deg_vals, return_counts=True)

    ax_hist.scatter(unique_degs, counts, color=METHOD_COLORS["RF (IC-BA)"], s=60,
    ...
    plt.tight_layout()
    out_path = _REPO_ROOT / "results/figures/networks/ba_network_structure.png"
