#!/usr/bin/env python
"""
scripts/experiments/train_rf_si_ba.py
=====================================
Train a Random Forest on node structural features to predict Patient Zero
in SI cascades on 200-node Barabási–Albert graphs.
...
Usage
-----
    python -m scripts.experiments.train_rf_si_ba
"""
...
def generate_data(seed: int, cascade_size: int) -> tuple[list[CascadeResult], list[float]]:
    ...
    print(f"    BA network: n={G.number_of_nodes()}, m={BA_M}, avg_deg={avg_deg:.2f}")
...
def print_results(avg_metrics: dict, cascade_size: int) -> None:
    ...
    print(f"\n{'='*105}")
    print(f"  BA + SI | CASCADE SIZE = {cascade_size} | AVERAGED OVER {len(SEEDS)} SEEDS")
    print(f"{'='*105}")
...
def main() -> None:
    ...
    mlflow.set_experiment("Random_Forest_BA_SI")

    for cascade_size in CASCADE_SIZES:
        print(f"\n\n{'#'*80}")
        print(f"  RUNNING BA + SI | CASCADE SIZE = {cascade_size}")
        print(f"{'#'*80}")
        ...
        with mlflow.start_run(run_name=f"BA_SI_size{cascade_size}_{len(SEEDS)}seeds"):
            mlflow.log_params({
                "N_NODES": N_NODES,
                "BA_M": BA_M,
            ...
            model_dir = Path("results/models/si_ba")
...
def _plot_accuracy(avg_metrics: dict, cascade_size: int) -> Path:
    ...
    for ax, k_measure, title in zip(axes, [1, 3],
            [f"Top-1 Accuracy — SI on BA (Mean over {len(SEEDS)} seeds, size={cascade_size})",
             f"Top-3 Accuracy — SI on BA (Mean over {len(SEEDS)} seeds, size={cascade_size})"]):
    ...
    plt.tight_layout()
    out_file = OUT_DIR / f"rf_vs_baselines_ba_si_size{cascade_size}.png"
...
def _plot_feature_importances(avg_importances: dict, cascade_size: int) -> Path | None:
    ...
    ax.set_title(f"Random Forest — Feature Importances (SI on BA, size={cascade_size})",
                 color="white", fontweight="bold")
    ax.tick_params(colors="lightgray")
    for sp in ax.spines.values(): sp.set_edgecolor("#444")

    plt.tight_layout()
    out_file = OUT_DIR / f"rf_feature_importance_ba_si_size{cascade_size}.png"
