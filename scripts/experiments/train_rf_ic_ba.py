#!/usr/bin/env python
"""train rf on ic-ba cascades across 5 seeds, save rf-vs-baselines bars and feature importances"""
...
CASCADE_SIZES = [25, 30]     # size 30 is rarer at low r0, see plot_cascade_size_threshold
N_TARGET      = 1000         # cascades per r0 (each must be >= cascade_size)
...
def print_results(avg_metrics: dict, cascade_size: int) -> None:
    ...
    print(f"\ncascade size = {cascade_size}, averaged over {len(SEEDS)} seeds")
...
def _plot_accuracy(avg_metrics: dict, cascade_size: int) -> Path:
    ...
    # "random_forest" here is the ic-ba model so it picks up the ba green
    palette = {
        "random_forest": METHOD_COLORS["RF (IC-BA)"],
...
def _plot_feature_importances(avg_importances: dict, cascade_size: int) -> Path | None:
    ...
    ax.barh(y_pos, scores, align='center', color=METHOD_COLORS["RF (IC-BA)"],
            edgecolor="black")
    ax.set_yticks(y_pos)
