#!/usr/bin/env python
"""
Plot IC on BA synthetic testset evaluation results across all R0 values.
Uses consistent color mapping: RF variants are greener.

Usage: python scripts/experiments/plot_metrics_ic_ba.py
"""
...
def main() -> None:
    ...
    for ax, key, title in zip(
        ...
        ["top1", "top3"],
        [
            f"top-1 accuracy vs $R_0$ -- IC on BA (over {n_seeds} seeds)",
            f"top-3 accuracy vs $R_0$ -- IC on BA (over {n_seeds} seeds)",
        ],
    ):
        for m in present:
            vals = []
    ...
    plt.tight_layout()
    out_file = OUT_DIR / "ic_ba_accuracy_vs_r0.png"
