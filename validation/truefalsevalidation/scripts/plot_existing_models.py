#!/usr/bin/env python
"""
Plot TrueFalse (Fake News) validation results from saved JSON.
Matches the style of Weibo and OutbreakTrees plots.
Generates:
  1. truefalse_accuracy.png
  2. truefalse_hops.png
  3. truefalse_hops_cumulative.png
...
def _accuracy_panel(ax, present, metrics, labels, mean_key, std_key, title):
    ...
    stds = []
    for m in present:
        vals.append(metrics[m].get(mean_key, 0.0))
        stds.append(metrics[m].get(std_key, 0.0))
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [labels.get(m, m) for m in present],
...
def _hop_panel(ax, present, hop_dist, labels, title):
    ...
    for i, m in enumerate(present):
        ...
        ax.bar(x + offset, ys, bar_w,
               label=labels.get(m, m), facecolor=color,
...
def main() -> None:
    ...
    sel_min       = data.get("selected_min", 88)
    sel_max       = data.get("selected_max", 92)
    sel_median    = data.get("selected_median", 90.0)
    ...
    mean_hops_line = "Mean hops: " + "  ".join(
        f"{labels.get(m, m)}={metrics[m]['mean_hops']:.2f}" for m in present_acc
    )
    fig1.text(
        0.5, 0.02,
    ...
    acc_out = OUT_DIR / "truefalse_accuracy.png"
    ...
    plt.tight_layout()
    hop_out = OUT_DIR / "truefalse_hops.png"
    ...
    _hop_cumulative_panel(
        ...
        f"cumulative hop distance -- TrueFalse ({range_title})",
    )
    plt.tight_layout()
    hop_cum_out = OUT_DIR / "truefalse_hops_cumulative.png"
