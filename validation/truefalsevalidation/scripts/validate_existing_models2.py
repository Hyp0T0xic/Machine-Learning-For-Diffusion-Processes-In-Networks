#!/usr/bin/env python
"""evaluate pre-trained ic-ba and ic-er rfs on real falsenews cascades (single-pass variant), dump metrics to json"""
...
def _compute_hop_distribution(cascades, rankings, max_hop_bin=4):
    """Bin hop distances into 0, 1, 2, 3, 4+ and return counts + percentages."""
    hop_counts = defaultdict(int)
    for c, r in zip(cascades, rankings):
        d = distance_to_source(c, r)
        if d == float("inf"):
            continue
        bin_h = int(min(d, max_hop_bin))
        hop_counts[bin_h] += 1
...
def main() -> None:
    ...
    # Compute per-method hop distributions
    hop_distributions = {}
    for method in all_rankings:
        hop_distributions[method] = _compute_hop_distribution(cascades, all_rankings[method])

    # save to JSON (standardized format matching other evaluation scripts)
    json_data = {
    ...
    for method, m in metrics.items():
        json_data["metrics"][method] = {
            "top1_mean": float(100 * m["top_k"][1]),
            "top1_std": 0.0,
            "top3_mean": float(100 * m["top_k"][3]),
            "top3_std": 0.0,
            "mrr_mean": float(m["mrr"]),
            "mrr_std": 0.0,
            "mean_hops": float(m["mean_distance"]),
        }
